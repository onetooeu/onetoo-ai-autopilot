 ...
 * Source: https://portal.onetoo.eu/ams/v1/envelopes?thread=t_contrib_submit
 * Output:
...
function readTextIfExists(p) { try { return fs.readFileSync(p, "utf8"); } catch { return null; } }
...
async function main() {
  ...
  const sandbox = prevSandbox && Array.isArray(prevSandbox.items) ? prevSandbox : {
    ok: true, kind: "contrib-sandbox", version: "v1",
    updated_at: nowIso(), source: { ams_base: base, thread: THREAD }, items: []
  };
  const rejected = prevRejected && Array.isArray(prevRejected.items) ? prevRejected : {
    ok: true, kind: "contrib-rejected", version: "v1",
    updated_at: nowIso(), source: { ams_base: base, thread: THREAD }, items: []
  };
...
main().catch(err => { console.error("[ingest] ERROR:", err?.stack || err); process.exit(1); });
